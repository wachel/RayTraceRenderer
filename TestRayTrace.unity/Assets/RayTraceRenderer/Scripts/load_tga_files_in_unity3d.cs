		...
		
	{
		...
		private void LoadTGAImage(BinaryReader binReader)
		{
			...
			if ( pf == TextureFormat.BGRA32 )
			{
                ...
                bmpTargaImage = new UnityEngine.Texture2D((int)this.objTargaHeader.Width, (int)this.objTargaHeader.Height, TextureFormat.RGBA32, false);
			}
            else if (pf == TextureFormat.RGB24) {
                for (int i = 0; i < bimagedata.Length; i += 3) {
                    // Swap R (byte0) with B (byte2)
                    byte r = bimagedata[i];
                    bimagedata[i] = bimagedata[i + 2];
                    bimagedata[i + 2] = r;
